...
export class AurumKeyboard {
	...
	public get disposed(): boolean {
		return this.cancelationToken.isCanceled;
	}

	private cancelationToken: CancellationToken;
	private heldDownKeys: { [key: number]: boolean };

	constructor() {
		this.cancelationToken = new CancellationToken();

		this.cancelationToken.registerDomEvent(document, 'keydown', (e: any) => {
			this.heldDownKeys[e.keyCode] = true;
		});
		this.cancelationToken.registerDomEvent(document, 'keyup', (e: any) => {
			delete this.heldDownKeys[e.keyCode];
		});
	...
	public isKeyDown(key: KeyboardButtons): boolean {
		return this.heldDownKeys[key];
	}
